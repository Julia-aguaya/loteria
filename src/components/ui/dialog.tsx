...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-4xl -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-[1.5rem] border border-border bg-card p-4 shadow-[0_30px_80px_rgba(15,23,42,0.24)] duration-200 sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)] sm:gap-5 sm:rounded-[1.75rem] sm:p-7',
        className,
      )}
      {...props}
    >
      {children}
      <DialogPrimitive.Close className="absolute right-3 top-3 inline-flex h-11 w-11 items-center justify-center rounded-full text-muted-foreground transition hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:right-4 sm:top-4">
        <X className="h-4 w-4" />
